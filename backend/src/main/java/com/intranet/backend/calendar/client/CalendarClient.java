...
public class CalendarClient {
    ...
    public CalendarApiResponse getHolidays(String country, int year) {
        ...
        try {
            CalendarApiResponse response = webClient.get()
                    .uri(uriBuilder -> uriBuilder
                            .path("/holidays")
                            .queryParam("api_key", calendarConfig.getApiKey())
                            .queryParam("country", country)
                            .queryParam("year", year)
                            .queryParam("language", calendarConfig.getDefaultLanguage())
                            .build())
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, clientResponse -> {
                        log.error("Erro 4xx ao buscar feriados: {}", clientResponse.statusCode());
                        return clientResponse.bodyToMono(String.class)
                                .flatMap(error -> {
                                    log.error("Detalhes do erro: {}", error);
                                    return reactor.core.publisher.Mono.error(
                                            new ExternalApiException("Erro ao buscar feriados: " + error)
                                    );
                                });
                    })
            ...
            log.info("Feriados obtidos com sucesso: {} feriados encontrados", 
                    response.getResponse().getHolidays() != null ? 
                    response.getResponse().getHolidays().size() : 0);

            return response;

        } catch (WebClientResponseException.NotFound e) {
            log.error("Recurso não encontrado: {}", e.getMessage());
            throw new ExternalApiException("Dados de calendário não encontrados para o país: " + country);
        } catch (WebClientResponseException.TooManyRequests e) {
            log.error("Limite de requisições excedido para API de calendário");
            throw new ExternalApiException("Limite de requisições excedido. Tente novamente mais tarde.");
