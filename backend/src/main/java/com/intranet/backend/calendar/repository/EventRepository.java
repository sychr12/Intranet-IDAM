...
public interface EventRepository extends JpaRepository<Event, String> {
    ...
    @Query(
            "SELECT e FROM Event e WHERE e.startDateTime BETWEEN :start AND :end AND e.isActive = true")
    List<Event> findEventsBetweenDates(
            @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query(
            "SELECT e FROM Event e WHERE e.startDateTime >= :date AND e.isActive = true ORDER BY e.startDateTime")
    ...
    @Query(
            "SELECT e FROM Event e WHERE e.startDateTime <= :date AND e.endDateTime >= :date AND e.isActive = true")
