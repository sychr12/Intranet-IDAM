...
public interface HolidayRepository extends JpaRepository<Holiday, String> {
    ...
    @Query(
            "SELECT h FROM Holiday h WHERE h.country = :country AND YEAR(h.date) = :year AND MONTH(h.date) = :month")
    List<Holiday> findByCountryAndYearAndMonth(
            @Param("country") String country, @Param("year") int year, @Param("month") int month);

    @Query(
            "SELECT h FROM Holiday h WHERE h.country = :country AND h.date BETWEEN :startDate AND :endDate")
    List<Holiday> findBetweenDates(
    ...
    List<Holiday> findUpcomingHolidays(
            @Param("country") String country, @Param("date") LocalDate date);
    ...
    List<Holiday> findByDateAndCountry(
            @Param("date") LocalDate date, @Param("country") String country);
